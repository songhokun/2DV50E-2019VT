...
Furthermore, authentication mechanisms are missing on Phase 2 \cite{I-D.bortzmeyer-dprive-step-2}, and the lack of authentication of the authoritative server may potentially enable a Man-in-the-middle attack.
...
As DoH\cite{rfc8484} and DoT\cite{hu2016specification} use TLS protocol for encryption, an authoritative server processing multiple TLS session is likely to end up exhausting its computational resources, similar to a Denial-of-service attack situation \cite{bhople2012server}. The issues are well discussed in the internet draft "Next step for DPRIVE: resolver-to-auth link \cite{I-D.bortzmeyer-dprive-step-2}."
