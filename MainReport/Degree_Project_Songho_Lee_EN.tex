...
A systematic literature review was performed to accomplish \textbf{O1}; to study mitigative methods of DNS privacy. 
A search criterium was set to list articles that cited RFC 7626 from a database Google Scholar, as the analysis had provided a clear insight of DNS privacy issues\cite{rfc7626}.
Since around four years had passed after its publication, it was anticipated that fellow researchers have tried to solve or list risks identified in the article.
...
\textbf{O2} is accomplished by selecting securing methods that are near or already in practice and reproduce these in a controlled environment.
Due to this reason, it may exclude some of the areas from \textbf{O1}. Verification, as defined in \textbf{O2}, is done by Examining DNS query and response packets between stub resolver and recursive resolver, after having applied privacy enhancive methods under the controlled environment. If the time allows, Deep Packet Inspection(DPI) middleware could optionally be deployed in the environment and see results of analysis of the secured DNS query process.
...
Improvement suggestions were further classified into ones securing the content and others enhancing the channel.
...
\FloatBarrier
\subsection{Analysis}
RFC 7626 identified risk area of DNS privacy as the followings\cite{rfc7626}: 
\begin{enumerate}
...
\end{enumerate}
We found many studies and standardisations which attempt to alleviate risks on the wire (2), as we denoted securing communication channel in Table \ref{content}. Securing the channel also partially addresses risks of (4) re-identification depends on who the subject of the attacker is. 
...
\subsection{Why securing DNS when vulnerabilities from other compartment exist?}
