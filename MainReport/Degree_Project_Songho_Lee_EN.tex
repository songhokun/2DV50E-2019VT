...
\begin{abstract}
\noindent Den nuvarande användningen av DNS-systemet är det mest signifikanta kryphålet för Internetanvändares integritet, eftersom alla frågor och svar som krävs för att konvertera en webbadress till IP-adress inte skyddas i de flesta fall.
Rapporten identifierade Internetanvändarnas integritetsintressen och presenterade den nuvarande tekniken som syftar till att förbättra DNS-sekretessen genom en systematisk litteraturgranskning.
Rapporten undersökte också begränsningarna i den nuvarande praxis och redovisade flera förslag såsom DNS-över-Tor och metoder som möjliggör periodiskt ändring av rekursiva resolvrar, och de metoderna förväntas att minimera integritetsläckor.
\\\\
